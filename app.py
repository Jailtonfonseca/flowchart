...
def run_orchestrator(task: str, llm_config: Dict[str, Any], verifier: Verifier, auto_apply: bool, max_agents: int, capture: CaptureBuffer) -> None:
...
def render_app() -> None:
    ...
    api_key = st.sidebar.text_input("OPENROUTER_API_KEY", type="password", value=os.getenv("OPENROUTER_API_KEY", ""))
    ...
    if st.button("Construir Equipe e Executar"):
        ...
        threading.Thread(target=run_orchestrator, kwargs={"task": task, "llm_config": llm_config, "verifier": verifier, "auto_apply": auto_apply, "max_agents": max_agents, "capture": capture}, daemon=True).start()
